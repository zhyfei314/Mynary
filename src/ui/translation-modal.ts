...
export class TranslationModal extends Modal {
	...
	onOpen() {
		...
		if (!options.some((language) => language.code === this.plugin.settings.mtranTargetLanguage)) target.createEl('option', { value: this.plugin.settings.mtranTargetLanguage, text: this.plugin.settings.mtranTargetLanguage.toUpperCase() });
		target.value = this.plugin.settings.mtranTargetLanguage;
		...
		target.addEventListener('change', () => { this.plugin.settings.mtranTargetLanguage = target.value; void this.plugin.saveSettings(); run(); });
